...
import { type Attachment, ListAttachmentsRequestSchema } from "@/types/proto/api/v1/attachment_service_pb";
...
const Attachments = () => {
  ...
  useEffect(() => {
    ...
    documentItems.forEach((item) => {
      if (getDocumentPreviewMode({ mimeType: item.attachment.type, filename: item.attachment.filename }) !== "fallback") {
        prewarmDocumentThumbnail({ sourceUrl: item.sourceUrl, filename: item.attachment.filename, mimeType: item.attachment.type });
      }
    });
  ...
  const handleOpenDocument = (item: AttachmentLibraryListItem) => {
    setPreviewDocument(item.attachment);
  };
  ...
  const renderContent = () => {
    ...
    if (activeTab === "documents") {
      return <AttachmentDocumentRows items={documentItems} onOpenDocument={handleOpenDocument} />;
    }
