...
const DESKTOP_EXPLORER_CLASS_NAME = cn(
  "sticky top-0 h-svh shrink-0 border-r border-border/60 bg-sidebar/72 backdrop-blur-xl transition-all",
...
const MainLayout = () => {
  ...
  const context: MemoExplorerContext = useMemo(() => {
    ...
    if (location.pathname === Routes.RANDOM) return "home";
    if (location.pathname === Routes.INSIGHTS) return "home";
  ...
  return (
    <section className="@container w-full min-h-full flex flex-col justify-start items-center md:flex-row md:items-start">
      ...
      <div className={MAIN_CONTENT_CLASS_NAME}>
        <div className={cn("w-full mx-auto px-3 sm:px-5 md:px-8 md:pt-7 pb-10")}>
          <Outlet />
        </div>
      </div>
    </section>
