...
export const useFileUpload = (onFilesSelected: (localFiles: LocalFile[]) => void) => {
  ...
  const handleFileInputChange = (event?: React.ChangeEvent<HTMLInputElement>) => {
    const input = fileInputRef.current ?? event?.currentTarget;
    ...
    try {
      ...
      localFiles.forEach((localFile) => {
        if (getDocumentPreviewMode({ mimeType: localFile.file.type, filename: localFile.file.name }) !== "fallback") {
          prewarmDocumentThumbnail({ sourceUrl: localFile.previewUrl, filename: localFile.file.name, mimeType: localFile.file.type });
        }
      });
