...
import { normalizeEditorContentForSave } from "../Editor/formatting";
...
function buildUpdateMask(
  ...
  const normalizedContent = normalizeEditorContent(normalizeEditorContentForSave(state.content));
  const patch: Partial<Memo> = {
    name: prevMemo.name,
    content: normalizedContent,
  };

  if (!isEqual(normalizedContent, prevMemo.content)) {
    mask.add("content");
...
export const memoService = {
  async save(
    ...
    const memoData = create(MemoSchema, {
      content: normalizeEditorContent(normalizeEditorContentForSave(state.content)),
  ...
  fromMemo(memo: Memo): EditorState {
    return {
      content: normalizeEditorContent(memo.content),
      metadata: {
