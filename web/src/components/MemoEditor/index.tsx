...
import { EDITOR_UI_SYNC_EVENT, type EditorRefActions } from "./Editor";
...
const MemoEditorImpl: React.FC<MemoEditorProps> = ({
  ...
  const keepEditorActive = useCallback(() => {
    setIsEditorFocusPinned(true);
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        setIsEditorFocusPinned(false);
      });
    });
  }, []);
  ...
  useEffect(() => {
    const syncInlineStyles = () => {
      try {
        const editor = editorRef.current;
        const editorElement = editor?.getEditor();
        const activeElement = document.activeElement;
        if (!editor || !editorElement || !activeElement || (activeElement !== editorElement && !editorElement.contains(activeElement))) {
          setActiveInlineStyles({ bold: false, italic: false });
          return;
        }

        setActiveInlineStyles({
          bold: editor.isInlineStyleActive("bold"),
          italic: editor.isInlineStyleActive("italic"),
        });
      } catch (error) {
        console.error("Failed to sync inline styles:", error);
        setActiveInlineStyles({ bold: false, italic: false });
      }
    };

    syncInlineStyles();
    document.addEventListener("selectionchange", syncInlineStyles);
    document.addEventListener(EDITOR_UI_SYNC_EVENT, syncInlineStyles as EventListener);

    return () => {
      document.removeEventListener("selectionchange", syncInlineStyles);
      document.removeEventListener(EDITOR_UI_SYNC_EVENT, syncInlineStyles as EventListener);
    };
  }, []);
  ...
  async function handleSave() {
    ...
    const { valid, reason } = validationService.canSave(state, contentLengthLimit);
    if (!valid) {
    ...
    try {
      ...
      editorRef.current?.resetToInitialState();
  ...
  return (
    <>
      ...
      */}
      <div
        className={cn(
          "group relative w-full flex flex-col justify-between items-start bg-[var(--memo-surface)] px-3 pt-3.5 pb-3 rounded-[0.85rem] border border-border/65 gap-3 shadow-[var(--memo-shadow)] transition-[border-radius,padding,box-shadow] duration-200 sm:px-3.5",
