...
const Navigation = (props: Props) => {
  ...
  return (
    <header className={cn("w-full h-full overflow-auto flex flex-col justify-between items-start gap-5", className)}>
        ...
        <TooltipProvider>
          {primaryNavLinks.map((navLink) => (
            <NavLink
              className={({ isActive }) =>
                cn(
                  "px-2.5 py-2.5 rounded-[1.25rem] border flex flex-row items-center text-base font-medium text-sidebar-foreground transition-all",
                  collapsed ? "" : "w-full px-4",
                  isActive
                    ? "bg-sidebar-accent text-sidebar-accent-foreground border-border/55 shadow-sm"
                    : "border-transparent hover:bg-sidebar-accent/82 hover:text-sidebar-accent-foreground hover:border-border/40 opacity-85",
                )
              }
              key={navLink.id}
              to={navLink.path}
      ...
      {currentUser && (
        <div className={cn("w-full flex flex-col justify-end pb-3", props.collapsed ? "items-center" : "items-start pl-3")}>
