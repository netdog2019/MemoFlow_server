...
func (r *renderer) renderFieldPredicate(cond *FieldPredicateCondition) (renderResult, error) {
	...
	case FieldKindAttachment:
		return r.renderAttachmentPredicate(field), nil
	default:
...
func (r *renderer) renderComparison(cond *ComparisonCondition) (renderResult, error) {
	...
	case *FieldRef:
		...
		case FieldKindAttachment:
			return r.renderAttachmentComparison(field, cond.Operator, cond.Right)
...
func (r *renderer) renderAttachmentComparison(field Field, op ComparisonOperator, right ValueExpr) (renderResult, error) {
	value, err := expectBool(right)
	if err != nil {
		return renderResult{}, err
	}
	if op != CompareEq && op != CompareNeq {
		return renderResult{}, errors.Errorf("operator %s not supported for attachment field", op)
	}

	result := r.renderAttachmentPredicate(field)
	shouldNegate := (op == CompareEq && !value) || (op == CompareNeq && value)
	if shouldNegate {
		return renderResult{sql: fmt.Sprintf("NOT (%s)", result.sql)}, nil
	}
	return result, nil
}
...
func (r *renderer) renderAttachmentPredicate(field Field) renderResult {
	memoIDColumn := qualifyColumn(r.dialect, Column{Table: "memo", Name: "id"})
	attachmentMemoIDColumn := qualifyColumn(r.dialect, Column{Table: "attachment", Name: "memo_id"})
	attachmentTypeColumn := qualifyColumn(r.dialect, Column{Table: "attachment", Name: "type"})
	conditions := []string{fmt.Sprintf("%s = %s", attachmentMemoIDColumn, memoIDColumn)}

	if field.MimePrefix != "" {
		conditions = append(conditions, fmt.Sprintf("%s LIKE %s", attachmentTypeColumn, r.addArg(field.MimePrefix+"%")))
	}

	return renderResult{
		sql: fmt.Sprintf("EXISTS (SELECT 1 FROM %s WHERE %s)", tableName(r.dialect, "attachment"), strings.Join(conditions, " AND ")),
	}
}
...
func tableName(d DialectName, name string) string {
